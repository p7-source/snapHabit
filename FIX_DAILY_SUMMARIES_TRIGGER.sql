-- FIX DAILY_SUMMARIES TRIGGER FOR CLERK
-- This fixes the trigger function to work with TEXT user_id and the date column
...
BEGIN
  ...
  ELSE
    RAISE NOTICE '⚠️ daily_summaries.user_id column not found';
  END IF;
EXCEPTION
  WHEN OTHERS THEN
...
BEGIN
  ...
  -- Use date column if available, otherwise use DATE(created_at)
...
-- Done! ✅
-- The trigger should now work with Clerk user IDs (TEXT format)
